...
### C2.3/C2.4 amendment 3 (3 Sep 2026) — the coach can create exercises the library lacks, and reasons from real exercise knowledge (anatomy, rep ranges, skill progressions) rather than only what the digest shows. `js/coach-api.js`: a `newExercises` array — `[{key, name, muscleGroup, equipment, exerciseType, targets}]`, every field a plain required string, no enum, no new anyOf — is added to the `plan` schema (so it rides inside `$defs/plan` for session/chat too, keeping the "PLAN v2 emitted identically in all three places" invariant intact for free) and, separately, as its own top-level property on `SESSION_SCHEMA`/`CHAT_SCHEMA`, so a session or chat reply can create an exercise even when `plan` is null (e.g. to reference it from `better`/`worse`). `parseResponse` reads the top-level `newExercises` for kind 'session'/'chat' and the plan object's own field for kind 'plan' (raw IS the plan there); a redundant nested `plan.newExercises` inside a session/chat reply is never read — `cleanPlan` only ever copies the fields it already knew about. `cleanNewExercises` validates each entry (≤8, `key`/`name` deduped — name case-insensitively, a name under 2 chars after trimming drops the entry — `name` truncates to 60 chars, `muscleGroup`/`equipment` fall back to 'other', `exerciseType` falls back to `weight_reps` against the nine ids in `js/exercise-types.js` minus `notes`, `targets` truncates to 120 chars and may be `''`) then the caller merges the result into the existing id→type map (`types.set('new:<key>', exerciseType)`) BEFORE cleaning plan/session/chat, so a `new:<key>` reference in a plan exercise, `planChanges[]`, `better[]` or `worse[]` validates exactly like any other known `exerciseId` — kept when it matches a validated `newExercises` entry (its `exerciseType` drives the existing duration/rep clamp unchanged), dropped as unknown otherwise. Every kind but `daily` now returns `newExercises` on the parsed narrative (`[]` when nothing was created). `SYSTEM_PROMPT` gains three sections — EXERCISE KNOWLEDGE (anatomy regions per movement, rep ranges by goal, overload levers, zone two cardio), SKILL GOALS AND PROGRESSIONS (build and overload a named progression for a stated skill goal, e.g. first pull-up), CREATING EXERCISES (add to `newExercises` and reference it as `exerciseId: "new:<key>"`, create only what the plan uses, say so in prose) — plus small edits to FORMAT, DATA CONVENTIONS and PLANNING RULES pointing at `newExercises`/`new:`; it stays a frozen, digest-independent constant, now under 12000 chars (raised from the 9000 test budget, itself raised from the original C0 budget, to fit the new sections). `js/coach.js`: `materialiseNewExercises(narrative)` reuses an existing exercise with the same name (case-insensitive, via `listExercises()`) or creates one (`putExercise({..., isCustom: true, isUnilateral: false, targets, createdBy: 'coach', createdAt: nowISO()})`), returning a `Map('new:<key>' → real id)` (plus a non-standard `.createdCount` for the exercises actually created, not matched); `rewriteNewExerciseIds(narrative, idMap)` then resolves every `new:` reference — plan `sessions[].exercises[].exerciseId` (handling both the bare-plan shape from kind 'plan' and the nested `.plan` from session/chat), `planChanges[].exerciseId`, `better[]`/`worse[].exerciseId` — before the narrative reaches `storePlan`/`putCoachInsight`/`putChatMessage`. Wired into `createPlan`, `runSessionFeedback` and `sendChat`; the chat coach message's `changed` object gains a fourth key, `exercises: idMap.createdCount`, alongside the existing `plan`/`profile`/`memory` booleans. `js/db.js`'s `ExerciseRecord` JSDoc gains two additive optional fields matching this: `targets: string|null` and `createdBy: 'user'|'coach'|null` (absent ⇒ user) — no code change, the store is already schemaless.
